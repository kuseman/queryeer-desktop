...
- Payloadbuilder engine state may include `payloadbuilder.sessionId`; it is nested under `payloadbuilder` and is not a root engine-state field.
...
- Payloadbuilder maintains a persistent `QuerySession` per file. Executions for one file are serialized because this session is mutable; executions for different files may run concurrently. Sessions persist across queries within the same file, allowing variables and temp tables to be maintained. Before each execution, the complete submitted catalog state and resolved environment variables are applied to the session. Catalog providers may translate frontend properties into different native session properties. After the completion patch has been extracted, each provider clears both its submitted and translated properties, and environment variables are restored to their pre-execution values, so request state cannot leak into the next execution or overwrite persistent session variables. The session is recreated only when catalog registry topology changes, meaning an alias or its `catalogId` is added, removed, or changed. Catalog-specific property names such as `connectionId` are not interpreted as session identity by the backend. The session is cleaned up when the file is closed. After execution, the backend reflects the current session counter in `queryengine.completed.engineState.payloadbuilder.sessionId` so the UI can display the session identifier in the tab title (e.g., `(1) filename.plbsql`). Completion engine state is a patch; the desktop applies it only while the complete environment and catalog configuration submitted by that execution is still current.
