...
- Message categories:
  ...
  - server notifications (`queryengine.progress`, `queryengine.resultChunk`, `queryengine.completed`, `queryengine.failed`)
- Correlation:
...
- Backend gateway mock scaffold added:
  ...
  - gateway supports first end-to-end mock query flow (`queryengine.execute` / `queryengine.cancel`)
...
- Java backend scaffold created: `queryeer-backend/`
  ...
  - stdio transport now handles first protocol methods (`backend.handshake`, `health.ping`,
    `queryengine.execute`, `queryengine.cancel`) with mocked notifications
