...
`queryengine.execute` requires `fileId`. The backend binds execution to the matching open file session and can reuse file-scoped state instead of re-parsing `text`.
